...
export class VoiceActivityDetector {
  ...
  detectVoiceActivity(segmentBuffer: ByteBuffer): VoiceEvent[] {
    ...
    if (segmentBuffer !== null && segmentBuffer.remaining() > 0) {
      if (segmentBuffer.remaining() % 2 !== 0) {
        throw new Error(
          "Voice Transcription SDK supports 8-bit audio only. segmentBuffer parameter must be with even number of bytes."
        );
      } else if (this.triggeredEndOfStream) {
        throw new Error(
          "VoiceActivityDetector was already notified of end of stream. Invoking detectVoiceActivity() is not allowed."
        );
      } else {
        if (
          this.segmentShorterThanMinimum !== null &&
          this.segmentShorterThanMinimum.remaining() > 0
        ) {
          segmentBuffer = ByteBuffer.concat([
            this.segmentShorterThanMinimum,
            segmentBuffer,
          ]);
          this.segmentShorterThanMinimum = null;
        }
        ...
        while (segmentBuffer.remaining() > 0) {
          if (segmentBuffer.remaining() >= this.bytesPer100MilliSec) {
            ...
            if (
              this.voiceEventsCounter == this.voiceEventsThreshold &&
              this.startPos < 0
            ) {
              this.startPos =
                this.currentSegmentStartIndex -
                this.voiceEventsThreshold * this.bytesPer100MilliSec;
              this.startPos = this.startPos < 0 ? 0 : this.startPos;
              this.silenceEventsCounter = 0;
            } else if (
              this.silenceEventsCounter == this.silenceEventsThreshold &&
              this.startPos > -1
            ) {
              const endPos =
                this.currentSegmentStartIndex -
                this.silenceEventsThreshold * this.bytesPer100MilliSec;
              voiceEvents.push(
                new VoiceEvent(this.startPos, endPos, this.audioBytesPerSec, 1)
              );
              this.voiceEventsCounter = 0;
              this.startPos = -1;
            }
  ...
  private getEventTypeOn8BitAudio(chunk: Uint8Array): EventType {
    ...
    return this.energyCalculator.getMaxEnergy() < 80 &&
      this.energyCalculator.getAvgEnergy() < 110
      ? EventType.VOICE
      : EventType.SILENCE;
  }
  ...
  public notifyEndOfStream(): VoiceEvent[] {
    ...
    if (this.startPos > -1) {
      const endPos =
        this.currentSegmentStartIndex - this.silenceEventsThreshold;
      voiceEvents.push(
        new VoiceEvent(this.startPos, endPos, this.audioBytesPerSec, 1)
      );
    }
  ...
  private getTypedArray(
    ...
    length: number = byteBuffer.remaining()
  ) {
