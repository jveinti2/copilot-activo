...
export class SessionWebsocketStatsTracker implements ServerWebSocket {
  ...
  createTrackingHandler(): OpenHandler {
    return ({ session, selectedMedia }) => {
      ...
      const resumeHandler: OnResumedHandler = (parameter) => {
        this.durationPaused = this.durationPaused.withAddedDuration(
          parameter.discarded
        );
      ...
      const discardedHandler: OnDiscardedHandler = (parameter) => {
        this.durationDiscarded = this.durationDiscarded.withAddedDuration(
          parameter.discarded
        );
        this.lastPosition = session.position;
      };
      ...
      const audioHandler: OnAudioHandler = (frame) => {
        const duration = StreamDuration.fromSamples(
          frame.sampleCount,
          frame.rate
        );
  ...
  summary(): SessionWebsocketStatsTrackerSummary {
    return {
      timing: {
        ...
        open: StreamDuration.fromMilliseconds(
          Math.max(0, this.openTimestamp - this.startTimestamp)
        ).asDuration(),
        close: StreamDuration.fromMilliseconds(
          Math.max(0, this.closeTimestamp - this.startTimestamp)
        ).asDuration(),
      },
      transport: {
        sent: {
  ...
  on(
    event: "close",
    listener: (this: ServerWebSocket, code: number) => void
  ): this;
  on(
    event: "error",
    listener: (this: ServerWebSocket, err: Error) => void
  ): this;
  on(
    event: "message",
    listener: (
