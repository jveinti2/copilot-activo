...
export type OnNewHealthCheckClientHandler = (
  remoteIp: string,
...
class ServiceLifecyleImpl implements ServiceLifecyle {
  ...
  handleHealthCheck(request: FastifyRequest): boolean {
    ...
    if (remoteIp) {
      ...
      if (!item) {
        this.healthCheckStats.set(remoteIp, {
          addr: remoteIp,
          tsFirst: now,
          tsLast: now,
          count: 1,
  ...
  drainAndClose(reason: string): void {
    console.log(`⚠️ Iniciando apagado del servicio. Motivo: ${reason}`);
    if (this.sessions.size > 0) {
      if (this.state !== "DRAINING") {
        console.log(
          `⏳ Servicio tiene ${this.sessions.size} sesiones activas, drenando...`
        );
        this.state = "DRAINING";
...
// Plugin para gestionar el ciclo de vida del servicio
const serviceLifecyclePlugin: FastifyPluginAsync<
...
> = async (fastify, options) => {
  ...
  fastify.ready((err) => {
    if (err) {
      ...
      console.log("✅ Servicio listo, estado RUNNING");
